...
public final class Statistics {
   ...
   *
   *
   * <h2>Stores a rotational estimation of something known to be coming from a normal distribution.
   * </h2>
   *
   * @param center the center of the estimation
  ...
  public static RotationEstimation rotationFilter(RotationEstimation... rotationEstimations) {
    ...
    List<Estimation> cosEstimations =
        Arrays.stream(rotationEstimations)
            .map(
                est ->
                    new Estimation(Math.cos(est.center().getRadians()), est.standardDeviationRad()))
    ...
    List<Estimation> sinEstimations =
        Arrays.stream(rotationEstimations)
            .map(
                est ->
                    new Estimation(Math.sin(est.center().getRadians()), est.standardDeviationRad()))
            .collect(Collectors.toList());
