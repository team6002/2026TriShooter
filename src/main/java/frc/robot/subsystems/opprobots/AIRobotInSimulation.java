...
public class AIRobotInSimulation extends SubsystemBase {
    ...
    public static final AIRobotInSimulation[] instances = new AIRobotInSimulation[5]; // you can create as many opponent robots as you needs
    public static void startOpponentRobotSimulations() {
        try {
            for (int i = 0; i < 5; i++) {
                ...
                Logger.recordOutput("OppRobot/Pose" + i, 
                    instances[i].driveSimulation.getDriveTrainSimulation().getSimulatedDriveTrainPose());
            }
        } catch (Exception e) {
            DriverStation.reportError("Failed to load opponent robot simulations: " + e.getMessage(), e.getStackTrace());
        }
    }
