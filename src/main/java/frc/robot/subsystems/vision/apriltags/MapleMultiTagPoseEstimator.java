...
public class MapleMultiTagPoseEstimator {
  ...
  private Optional<Pose3d> calculateRobotPose3dFromSingleObservation(
    ...
    return fieldLayout
        ...
        .map(
            tagPose ->
                tagPose.transformBy(cameraToTarget.inverse()).transformBy(robotToCamera.inverse()));
  }
  ...
  private boolean shouldDiscardTagObservation(
    ...
    Angle tagAngle = cameraToTagBackRotation.minus(cameraToTargetTranslationDirection).getMeasure();
    boolean angleTooBig = tagAngle.abs(Radians) > MAX_TAG_ANGLE.in(Radians);
    if (Robot.LOG_DETAILS) {
      String logPath =
          APRIL_TAGS_VISION_PATH + "Filtering/TagObservations/Camera-" + cameraID + "/Tag-" + tagID;
  ...
  private Optional<VisionObservation> getEstimationResultFromValidObservations(
    ...
    final Translation2d translationPointEstimate =
        new Translation2d(
            robotPoseFinalEstimationXMeters.center(), robotPoseFinalEstimationYMeters.center());
